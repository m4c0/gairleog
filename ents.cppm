...
namespace ents {
  ...
  hai::varray<t> g_data { 1024 };

  export void foreach(auto && fn) {
    for (auto & e : g_data) if (e.life) fn(e);
  }
  ...
  export void add(dotz::ivec2 pos, entdefs::t tdef) {
    g_data.push_back_doubling(create(pos, tdef));
  }
  ...
  export void read(file::reader * r) {
    ...
    for (auto i = 0; i < len; i++) {
      ...
      g_data.push_back_doubling(val);
    }
  }
  export void write(file::writer * w) {
    silog::infof("Storing %d entities", g_data.size());
    w->write(g_data.size());
